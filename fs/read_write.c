...
PUBLIC int do_rdwt()
{
	...
	if (imode == I_CHAR_SPECIAL) {
		...
		int bytes_rw = 0;
		/*-----------------------------------------------------------*
		 * B级修复:原实现 bytes_left 直接取 len(请求字节数),读取  *
		 * 时按整扇区拷贝,导致 read() 会返回超过文件实际大小的内容  *
		 * (文件尾部所在扇区的残留字节也被返回)。这会让 cat 等工具 *
		 * 读到 EOF 之后的垃圾数据(其中的 0x0C 等控制字符还会扰乱   *
		 * 屏幕)。这里把读操作的可读字节数限制为 (pos_end - pos),  *
		 * 即 min(len, i_size - pos),使 read() 在文件结尾正确返回。 *
		 *-----------------------------------------------------------*/
