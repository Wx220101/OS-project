 ...
 *****************************************************************************/
/**
 * <Ring 0> 选择下一个要运行的进程 —— 多级反馈队列(MLFQ)调度器。
 *
 * 【B 级原创:重写自原书简单的"最大剩余时间片"调度】
 *
 * 算法:
 *   1. 从第 0 层(最高优先级)向下逐层扫描;
 *   2. 在第一个"存在就绪且仍有时间片(ticks>0)进程"的层级里,
 *      采用 Round-Robin:从当前进程的下一个槽位开始环形查找,
 *      命中即作为 p_proc_ready 返回(保证同层公平轮转);
 *   3. 若所有就绪进程时间片都已用尽,则按各自所在层级为它们补充
 *      一个新的时间片,然后重新扫描(不会死循环:只要存在就绪进程)。
 *
 *****************************************************************************/
PUBLIC void schedule()
{
	...
	int		start = (int)(p_proc_ready - proc_table); /* 上次运行者下标 */

	for (;;) {
		/* 从最高优先级层级开始查找 */
		for (level = 0; level < NR_SCHED_LEVELS; level++) {
			int i;
			/* Round-Robin:从 start 的下一个槽位开始环形扫描 */
			for (i = 1; i <= n; i++) {
				p = &proc_table[(start + i) % n];
				if (p->p_flags == 0 &&		/* 就绪 */
				    p->ticks > 0 &&		/* 还有时间片 */
				    p->sched_level == level) {	/* 属于当前层级 */
					p_proc_ready = p;
					p->n_dispatch++;	/* 调度统计 */
					return;
				}
		...
		/* 走到这里:没有任何就绪进程还有时间片 —— 给所有就绪进程
		 * 按其所在层级重新发放时间片,然后回到循环顶部重新选择。 */
		for (p = &FIRST_PROC; p <= &LAST_PROC; p++)
			if (p->p_flags == 0)
				p->ticks = SCHED_QUANTUM(p->sched_level);
	}
}
 ...
 *****************************************************************************/
/**
 * <Ring 0> 优先级老化(防饿死):把所有活动进程提升回最高优先级层级。
 *
 * 【B 级原创】MLFQ 规则 5。由时钟中断每隔 SCHED_BOOST_TICKS 个 tick 调用,
 * 避免长期停留在低层的 CPU 密集型进程因得不到 CPU 而被饿死。
 *
 *****************************************************************************/
PUBLIC void sched_boost()
{
	struct proc* p;
	for (p = &FIRST_PROC; p <= &LAST_PROC; p++)
		if (!(p->p_flags & FREE_SLOT))	/* 所有已分配(非空闲)槽位 */
			p->sched_level = 0;
}
